...
router.post("/operate",async ctx=>{
  ...
  if(action == 'add'){
    ...
    try {
      const res = await User.findOneAndUpdate({userId},{mobile,job,state,roleList,deptId,md5(password)})
...
router.get("/emailcode",async ctx=>{
  ...
  let res = await  User.findOne({userEmail:email},"_id")
  ...
  if(!res){
    ...
    ctx.body = util.success(md5(code))
  }
})
