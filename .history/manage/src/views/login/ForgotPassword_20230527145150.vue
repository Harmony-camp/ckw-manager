      ...
      <div class="main">
        ...
        <div class="setPassword" v-if="active == 2">
          <el-form :model="setPasswd" :rules="rules" ref="resetPass">
            ...
            <el-form-item>
              <el-button type="primary" @click="ResetPass"> 下一步 </el-button>
              <el-button> 重置 </el-button>
            </el-form-item>
          </el-form>
        </div>
        <div class="success" v-if="active == 3">
          ...
          <div>设置成功，请从新登录</div>
          <el-button type="primary" @click="goLogin">从新登录</el-button>
...
import {GetEmailCode,UserSubmit} from '../../api'
import md5 from 'md5'
...
function ResetPass (){
  resetPass.value.validate(async (valid) => {
    if (valid) {
      if(setPasswd.password != setPasswd.rePassword) return ElMessage({type:"error",message:"两次输入的密码不一致"})
      let params = {password:setPasswd.password,action:"edit",...code.value}
      await UserSubmit(params)
      ElMessage({type:"success",message:"密码修改成功"})
      setTimeout(()=>{active.value = 3 },1500)
    }
  })
}
